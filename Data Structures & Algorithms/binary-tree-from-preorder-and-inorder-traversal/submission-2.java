...
class Solution{
    ...
    public TreeNode helper(int[] preorder, int preL, int preR, int[] inorder, int inL, int inR, HashMap<Integer, Integer> map){
        ...
        int rootVal = preorder[preL];// not 0
        int index = map.get(rootVal);
        int leftSize = index - inL;// - intL
        TreeNode root = new TreeNode(rootVal);
        root.left = helper(preorder, preL + 1, preL + leftSize, inorder, inL, index - 1, map);
        root.right = helper(preorder, preL + leftSize + 1, preR, inorder, index + 1, inR, map);
        return root;
    }
}
